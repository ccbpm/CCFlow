        ...
        cbx.combobox({
            ...
            "onSelect": function (p) {
                ...
                (function sel(n, KeyOfEn, FK_MapData) {
                    //保存选择的值.
                    Savcceval(FK_MapData, KeyOfEn, n);
...
function MakeCheckBoxsModel(mapExt, data, mapAttr) {
    ...
    textbox.css("height", "0px");
    var tbVal = textbox.val();
    if (tbVal == null) tbVal = "";
    //获取父节点
    var parentbox = textbox.parent();
    parentbox.css("height", "auto")
    //增加div 
    var div = $("<div style='width:65%;float:left;margin-top:10px'></div>");
    textbox.before(div);
    for (var i = 0; i < data.length; i++) {
        ...
        var lab = $("<span class='labRb align_cbl' style='font-size:14px' for='" + id + "'>&nbsp;" + en.Name + "</span>");
        div.append(lab);
    }
...
//设置值.
function Savcceval(fk_mapdata, keyOfEn, val) {
