...
function GenerFrm(FK_MapData) {
    ...
    //URL的表单
        ...
        mui('#CCForm_' + FK_MapData).load('http://www.baidu.com', { 
        }, function (response) { });
        return;
    }
    ...
    for (var j = 0; j < frmData.Sys_MapAttr.length; j++) {
        ...
        if ($('#CCForm_'+FK_MapData+' #TB_' + mapAttr.KeyOfEn).length == 1) {
            ...
            if (mapAttr.IsRichText == 0) {
                $('#CCForm_' + FK_MapData + ' #TB_' + mapAttr.KeyOfEn).text(defValue.replace(/"white-space: nowrap;"/g, ""));//只读大文本放到div里
            } else {
                $('#CCForm_' + FK_MapData + ' #TB_' + mapAttr.KeyOfEn).html(defValue.replace(/"white-space: nowrap;"/g, ""));//只读大文本放到div里
            }
...
function BindFrm(frmData,FK_MapData) {
    ...
    $("#CCForm_" + FK_MapData).html("").append(html);
    ...
    //展显
    $("#CCForm_" + FK_MapData).append(mapAttrsHtml);
...
function Save() {
    ...
    //判断内容是否保存到待办/保存表单的中相同字段，值拷贝到节点表单中
    var handler = new HttpHandler("BP.WF.HttpHandler.WF_MyFlow");
...
function InitMapAttr(Sys_MapAttr, groupID,frmData) {
    ...
    $.grep(Sys_MapAttr, function (item) {
        ...
        //图片签名
        if (attr.IsSigan == "1") {
            _html += "<div class='mui-input-row' id='Div_"+attr.KeyOfEn+"'>";
...
var FormUtils = {
    ...
    CreateTBString: function (attr,frmData) {
        ...
        if (attr.IsEnableQrCode && attr.IsEnableQrCode == "1") {
            html_string += "<div class='mui-input-row'  id='Div_" + attr.KeyOfEn +"'>";
        ...
        if (attr.UIContralType == 60) {
            html_string += "<div class='' style='padding:11px 15px;line-height:1.1;'  id='Div_" + attr.KeyOfEn +"'>";
        ...
        if (attr.UIHeight > 30) {
            ...
            if (attr.AtPara && attr.AtPara.indexOf("@IsRichText=1") >= 0) {
                ...
                html_string += "<textarea wrap='virtual' onpropertychange= 'this.style.posHeight=this.scrollHeight' cols='40' style='width:100%;border:solid 1px gray;' rows=\"5\" placeholder=\"" + strPlaceholder + "\" name=\"TB_" + attr.KeyOfEn + "\" id=\"TB_" + attr.KeyOfEn + "\"></textarea>";
            } else {
        ...
        if (attr.UIIsEnable == "0")
            ...
            html_string += "<input style='background-color:#fff' type='text' name=\"TB_" + attr.KeyOfEn + "\" id=\"TB_" + attr.KeyOfEn + "\" placeholder=\"" + strPlaceholder + "\" />";
    ...
    CreateTBDate: function (attr,frmData) {
        ...
        if (attr.UIIsEnable == "0"){
            ...
            inputHtml += "  <label name=\"LAB_" + attr.KeyOfEn + "\" id=\"LAB_" + attr.KeyOfEn + "\" data-options='{\"type\":" + attr.IsSupperText +"}' class='mui-pull-right ccformdate' style='text-align:right;min-width:220px;margin-top:7px'><p>请选择日期</p></label>";
        	inputHtml += "</a>";
    ...
    CreateTBDateTime: function (attr,frmData) {
        ...
        if (attr.UIIsEnable == "0"){
        	...
        	inputHtml += "<a class='mui-navigate-right'>";
            inputHtml += " <label name=\"LAB_" + attr.KeyOfEn + "\" id=\"LAB_" + attr.KeyOfEn + "\" data-options='{\"type\":" + attr.IsSupperText +"}' class='mui-pull-right ccformdate' style='text-align:right;min-width:220px;margin-top:7px'>请选择时间</label>";
        	inputHtml += "</a>";
        	inputHtml += "<input  type='hidden' name=\"TB_" + attr.KeyOfEn + "\" id=\"TB_" + attr.KeyOfEn + "\" />";	
        }
        return inputHtml;
    ...
    CreateDDLEnum: function (attr,frmData) {
        ...
        html_Select += "<select name=\"" + ctrl_ID + "\" id=\"" + ctrl_ID + "\"  " + (attr.UIIsEnable == "0" ? "disabled" : "") + " onchange = 'changeEnable(this,\"" + attr.FK_MapData + "\",\"" + attr.KeyOfEn + "\",\"" + attr.AtPara + "\",\"CCForm_" + attr.FK_MapData+"\")' >";
    ...
    CreateDDLPK: function (attr,frmData) {
        ...
        html_Select += "<select name=\"DDL_" + attr.KeyOfEn + "\" id=\"DDL_" + attr.KeyOfEn + "\" readonly='" + (attr.UIIsEnable == "0" ? "readonly" : "") + "' onchange = 'changeEnable(this,\"" + attr.FK_MapData + "\",\"" + attr.KeyOfEn + "\",\"" + attr.AtPara + "\",\"CCForm_" + attr.FK_MapData +"\")' >";
		...
		$.each(frmData.Sys_MapDtl, function (i, dtl) {
            if (gf.CtrlID == dtl.No) {
                if (dtl.MobileShowModel == undefined || dtl.MobileShowModel == 0) {
                    ...
                    dtlHtml += "<div style='background-color:#efeff4' class='mui-table-view-cell'>";
