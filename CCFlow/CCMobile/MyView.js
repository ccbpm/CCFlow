...
function GenerFrm() {
    ...
    //节点信息.
    var wf_node = frmData["WF_FrmNodeComponent"][0];

    //设置标题.
    document.getElementById("title").innerHTML = frmData.Sys_MapData[0].Name;
    //$("#titleBar").val(frmData.WF_Node[0].Name);

    //解析表单
    var node = frmData["WF_Node"][0];
    if (node.FormType == 2 || node.FormType == 3) {
        ...
        url += "&WorkID=" + GetQueryString("WorkID") + "&FK_Flow=" + GetQueryString("FK_Flow") + "&FK_Node=" + GetQueryString("FK_Node");

        var html = "<iframe ID='SelfForm' src='" + url + "' frameborder=0  style='width:100%; height:80%' leftMargin='0' topMargin='0' />";
...
var FormUtils = {
        ...
        if (webUser.CCBPMRunModel == 2)
            html_Sign += "<img name=\"Sign_" + attr.KeyOfEn + "\" id=\"Sign_" + attr.KeyOfEn + "\" src='../DataUser/Siganture/"+webUser.OrgNo+"/" + val + ".jpg' border=0 onerror=\"this.src='../DataUser/Siganture/UnName.jpg'\"/>";
        else
	    ...
	    CreateTBString: function (attr) {
            ...
            if (attr.UIContralType == 16 && attr.UIIsEnable == "0") {
                ...
                html_string += "<button type='button' class='mui-btn' id='TB_fixed'style='margin-right:20px;width:35%' onclick='GetFixedInfoByJDWD(\"" + frmData.MainTable[0].JD + "\",\"" + frmData.MainTable[0].WD + "\")'>系统定位</button>";
                html_string += "</div>";
                return html_string;
            }
            ...
            } else if (attr.UIIsEnable == 1) {
                html_string += "<div class='mui-input-row'>";
                html_string += "<label for=\"TB_" + attr.KeyOfEn + "\" ><p>" + attr.Name + "</p></label>";
            } else if (attr.UIIsEnable == "0") {
                html_string += "<div class='mui-input-row' style='height:auto'>";
                html_string += "<label for=\"TB_" + attr.KeyOfEn + "\" ><p>" + attr.Name + "</p></label>";
            }
	        
	        if (attr.UIIsEnable == "0")
                html_string += "<div  style='background-color:#fff;font-size:14px;float:right;display:inline;padding:10px 15px;'  name='TB_" + attr.KeyOfEn + "' id='TB_" + attr.KeyOfEn + "'></div>";
	        else
	    ...
	    CreateTBDateTime: function (attr) {
	        ...
	        if (attr.UIIsEnable == "0"){
	        	...
	        	inputHtml += "  <input name=\"TB_" + attr.KeyOfEn + "\" id=\"TB_" + attr.KeyOfEn + "\"  type='text' />";
	        }
	        return inputHtml;
	    },
...
//审核组件
function figure_Template_FigureFrmCheck(flowData) {
...
function TimeBase() {
    var url = "./WorkOpt/OneWork/TimeBase.htm?FK_Flow=" + GetQueryString("FK_Flow") + "&WorkID=" + GetQueryString("WorkID") + "&t=" + Math.random();
    SetHref(url);
}