...
function InitPage() {
    ...
    for (var i = 0; i < tracks.length; i++) {
        ...
        //内容.
        var doc = "";
        doc =  track.NDFromT + "     -  " + timeBase + track.ActionTypeText;
        ...
        if ((at == ActionType.Forward || ActionType.Start) && Hide_IsOpenFrm == true ) {
            doc += "<p><a href=\"javascript:ToFrm('" + track.NDFrom + "','" + track.WorkID + "','"+track.MyPK+"')\" >查看表单</a></p>";
        }
        ...
        var msg = track.Msg;
        if (msg != "")
            doc += "<font size='2' color=green>" + msg + "</font><br>";
        msg = msg.replace("WorkCheck@", "");
    ...
    if (gwls) {
        ...
        if (isHaveNoChecker == true) {
            ...
            var rowDay = "<tr>";
            rowDay += "<td colspan=3 class='TDDay'>等待审批</td>";
            rowDay += "</tr>";
            $("#Table1 tr:last").after(rowDay);
            ...
            for (var i = 0; i < gwls.length; i++) {
                ...
                var left = "";
                left += "<br><img src='../../../DataUser/UserIcon/" + gwl.FK_Emp + ".png'  onerror=\"src='../../../DataUser/UserIcon/Default.png'\" style='width:60px;' /><br>";
                left += "" + gwl.EmpName;

                var newRow = "";
                newRow = "<tr  title='等待审批人员' >";
                newRow += "<td class='TDTime'style='font-family:\"Microsoft YaHei\";font-size:16px' >" + left + "</td>";
                newRow += "<td class='TDBase' ></td>";
                newRow += "<td class='TDDoc' style='font-family:\"Microsoft YaHei\";font-size:16px'>" + doc + "</td>";
                newRow += "</tr>";
...
function UnSend() {
    mui.confirm('您确定要撤销发送吗？', function (e) {
        if (e.index == 1) {
            ...
            var url = "../../MyFlow.htm?FK_Flow=" + fk_flow + "&WorkID=" + workid + "&t=" + Math.random();
            SetHref(url);
        } 
    })
}

function ToFrm(ndFrom,workID,trackID) {
    ...
    var isShow = GetQueryString("IsShowUnSend");
    if (isShow == null || isShow == '')
        isShow = "0";

    url = "../../MyFrm.htm" + url + "&IsShowUnSend=" + isShow;
    SetHref(url);
}
