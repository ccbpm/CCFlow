...
function DealMenuUrl(menu) {
    ...
    if (menu.Url == undefined) menu.Url = menu.UrlExt;
    ...
    if (menu.MenuModel == "FlowNewEntity") {
        if (menu.Mark == "StartFlow")
            menu.Url = basePath+"/WF/CCMobile/CCBill/Opt/StartFlowByNewEntity.htm?FK_Flow=" + menu.Tag1 + "&MenuNo=" + menu.No;
        if (menu.Url.indexOf("../CCBill") != -1)
            menu.Url = "../CCMobile/CCBill/Opt/StartFlowByNewEntity.htm?FK_Flow=" + menu.Tag1 + "&MenuNo=" + menu.No;
    ...
    if (menu.MenuModel === "Dict") {
        if (menu.ListModel === 0) //如果是批量编辑模式.
            ...
            menu.Url = basePath + "/CCMobile/CCBill/SearchDict.htm?FrmID=" + menu.UrlExt;
        return menu;
    }
    ...
    if (menu.MenuModel === "DBList") {
        menu.Url = basePath + "/CCMobile/CCBill/SearchDBList.htm?IsReadonly=1&FrmID=" + menu.UrlExt;
        return menu;
    }
    if (menu.MenuModel === "Tabs") {
        menu.Url = basePath + "/WF/Portal/Tabs.htm?PageID=" + menu.No;
    ...
    if (menu.MenuModel === "Rpt3D" || menu.MenuModel == "Rpt3D") {
        menu.Url = basePath + "/CCFast/Rpt/Rpt3D.htm?PageID=" + menu.No;
        return menu;
    }
