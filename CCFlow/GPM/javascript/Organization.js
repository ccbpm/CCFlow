...
function SetSiganture() {
    var url = 'Siganture.htm?EmpNo=' + FK_Emp;
    WinOpen2(url, 'ss', 500, 400, 1);
}
...
function hasAuthority() {
    ...
    if (curNode) {
        ...
        $.messager.alert("警告", "获取部门节点错误：" + curNodeId, "warning");
        return false;
    }  //为空
}
//新增部门  
...
//上/下移（人员）  逻辑写到后台
function EmpFloatNode(v) {
    if (hasAuthority()) {
        Application.data.EmpFloatNode(v, $('#empAppGrid').datagrid('getSelected').NO, function (js, scope) {
            if (js == "false") {
                CC.Message.showError("操作提示", "操作失败！");
            } else {
                //加载列表
...
function doEdit() {
    if (hasAuthority()) {
        ...
        if (canEdit(SignType)) return;
        var personalID = $('#setPersonalID').val(); //身份证号.
    ...
    + setEamil + "," + setLeader + "," + setZwlb + "," + SignType + "," + personalID;
        //获取树数据
...
function initializeEmpTabs() {
    ...
    Application.data.getEmpInfo(curNodeId, FK_Emp, function (js, scope) {
        if (js) {
            ...
            $("#SignType").val(pushData.SignType);
            $("#setPersonalID").val(pushData.PersonalID);
    ...
    if (hasAuthority()) {
        Application.data.checkDeptDutyAndStation(curNodeId, function (js, scope) {
            if (js == "true") {
                ...
                $.messager.alert('提示', '添加/关联人员之前请先检查部门的岗位/职务是否已经设置', 'warning', checkDeptInfo);
                return;
            }
        }, this);
    }
...
//关联人员模糊查询
function connecteEmpSearchByNameOrDept(pageNumber, pageSize) {

    // 获取查询条件
    var searchText1 = $('#connecteEmp_shareText').val();
    if (searchText1 == "") {
        $.messager.alert("提示", "请输入查询姓名或者部门，支持姓名拼音、汉字，部门不支持拼音!", "info");
        return;
    }
    // 清空dataGrid列表数据
    $('#gridOtherDeptEmps').datagrid('loadData', { total: 0, rows: [] }); //清空下方DateGrid
    Application.data.getEmpsByNameOrDept(curNodeId, replaceTrim(searchText1), pageNumber, pageSize, function (js, scope) {
        if (js) {
            if (js == "") js = [];
            var pushData = eval('(' + js + ')')
            $('#gridOtherDeptEmps').datagrid({
            ...
            if (pg) {
                $(pg).pagination({
                    onRefresh: function (pageNumber, pageSize) {
                        connecteEmpSearchByNameOrDept(pageNumber, pageSize);
                    },
                    onSelectPage: function (pageNumber, pageSize) {
                        connecteEmpSearchByNameOrDept(pageNumber, pageSize);
                    }
        ...
        if (js) {
            ...
            $('#empAppGrid').datagrid({
                ...
                columns: [[
                       ...
                       { field: 'FK_DUTYTEXT', title: '职务', sortable: true, width: 160, align: 'center' },
                       { field: 'PERSONALID', title: '身份证号', sortable: true, width: 160, align: 'center' }
            ...
            if (pg) {
                $(pg).pagination({
                    onRefresh: function (pageNumber, pageSize) {
                        SearchByEmpNoOrName(pageNumber, pageSize); // 修改组织结构模糊查询不能实现分页：修改前 SearchByEmpNo_Name(pageNumber, pageSize)
                    },
                    onSelectPage: function (pageNumber, pageSize) {
                        SearchByEmpNoOrName(pageNumber, pageSize); // 修改组织结构模糊查询不能实现分页：修改前 SearchByEmpNo_Name(pageNumber, pageSize)
                    }
...
function addEmp() {
    ...
    infoStr += "," + $('#SignTypeNew').val();
    infoStr += "," + $('#TB_PersonalID').val(); //身份证号.
...
function LoadDataGridAdmin(pageNumber, pageSize) {
    ...
    var searchText = $('#searchText').val();
    Application.data.LoadDataGridDeptEmp(curNodeId, "Idx", replaceTrim(searchText), pageNumber, pageSize, function (js, scope) {
        if (js) {
            if (js == "") js = [];
            var pushData = eval('(' + js + ')');
