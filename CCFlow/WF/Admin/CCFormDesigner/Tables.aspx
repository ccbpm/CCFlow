<%@ Page Title="" Language="C#" MasterPageFile="~/WF/Admin/CCFormDesigner/Site.Master" AutoEventWireup="true" CodeBehind="Tables.aspx.cs" Inherits="CCFlow.WF.Admin.CCFormDesigner.Tables" %>
...
<!-- 徐淑豪: 2016-07-05 -->
<table>
<caption><a href='/WF/Comm/Sys/SFGuide.aspx?DoType=New&FromApp=SL'>新建</a> </caption>
<tr>
<th>序</th>
<th>数据源</th>
<th>表名</th>
...
<%
 ...
 <tr>
<td class="Idx" ><%=idx %></td>
<td><%=tab.FK_SFDBSrcT %></td>
<td><%=tab.No %></td>

<td><img src='<%=icon %>' /> <%=tab.Name %></td>

<td><%=tab.CodeStructT%></td>
...
<%
    ...
    if (src.DBSrcType != BP.Sys.DBSrcType.WebServices && tab.No.Contains("BP.")==false)
    {
        ...
        editDBLink = "<a href='/WF/MapDef/SFTableEditData.aspx?RefNo="+tab.No+"'>编辑("+dbNum+")</a>";
    }
...
<td><a href="/WF/MapDef/SFTable.aspx?RefNo=<%=tab.No %>&FromApp=SL">引用(<%=refNum %>)</a></td>

<td><a href="/WF/MapDef/SFTable.aspx?RefNo=<%=tab.No %>&FromApp=SL">编辑属性</a></td>
<td><%=editDBLink %></td>
<td><%=delLink %></td>
