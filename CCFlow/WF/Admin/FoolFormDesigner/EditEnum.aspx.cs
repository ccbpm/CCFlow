    ...
    public partial class Comm_MapDef_EditEnum : BP.Web.WebPage
    {
        ...
        public void BindEnum(MapAttr mapAttr)
        {
            ...
            if (this.MyPK != null)
            {
                isItem = this.Pub1.AddTR(isItem);
                this.Pub1.AddTDIdx(idx++);
                string html = "<a href=\"javascript:WinOpen('./MapExt/DDLFullCtrl.htm?FK_MapData=" + mapAttr.FK_MapData + "&RefNo=" + mapAttr.KeyOfEn + "&MyPK=" + mapAttr.FK_MapData + "_" + MapExtXmlList.DDLFullCtrl + "_" + mapAttr.KeyOfEn + "')\">下拉框自动完成</a>";
                html += " - <a href=\"javascript:WinOpen('./MapExt/ActiveDDL.htm?FK_MapData=" + mapAttr.FK_MapData + "&AttrOfOper=" + mapAttr.KeyOfEn + "&MyPK=" + MapExtXmlList.ActiveDDL + "_" + mapAttr.MyPK + "')\">设置级联动(如:省份,城市联动)</a>";
