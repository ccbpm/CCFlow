<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="DtlFixRow.aspx.cs" Inherits="CCFlow.WF.CCForm.DtlFixRow" %>
...
<head runat="server">
  ...
  <%--  <style type="text/css">
    ...
    <script type="text/javascript">
        function SumTreeData(parentNo, keyOfEn) {

            var treeData = $("#<%=hiddenRefData.ClientID %>").val().split(';');

            var parentCount = "0";

            for (var idx in treeData) {

                if (startWith(treeData[idx], parentNo + '@' + keyOfEn)) {
                    var filedID = treeData[idx].split('@')[2];
                    if (!startWith(filedID, "Pub1_"))
                        filedID = "Pub1_" + filedID;

                    var count = $("#" + filedID).val();

                    parentCount = parseFloat(replaceAll(count, ',', '')) + parseFloat(replaceAll(parentCount, ',', '')) + "";

                }
            }

            SumParent(replaceAll(parentCount, ',', ''), parentNo, keyOfEn);
        }

        function SumParent(count, no, keyOfEn) {
            $(":input").each(function (a, b) {
                var id = $(b).attr("id"); //获取id属性

                if (startWith(id, no + "_") && endWith(id, "_" + keyOfEn)) {
                    $(b).val(replaceAll(count, ',', ''));

                    var parentNo = id.split('_')[1];
                    $(":input").each(function (a1, b1) {
                        var parentId = $(b1).attr("id"); //获取id属性
                        if (endWith(parentId, "_" + parentNo + "_" + keyOfEn) && parentId != id) {
                            count = parseFloat(replaceAll(count, ',', '')) + parseFloat(replaceAll($(b1).val(), ',', '')) + "";
                        }
                    });
                    SumParent(replaceAll(count, ',', ''), parentNo, keyOfEn);
                }
            });
...
<body>
    ...
    <asp:TextBox ID="hiddenRefData" runat="server" Style="display: none;"></asp:TextBox>
    </form>
</body>
</html>
