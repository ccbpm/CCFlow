...
namespace CCFlow.WF
{
    public partial class TaskPoolSmal : BP.Web.WebPage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (dt.Rows.Count == 0)
            {
                this.Pub1.AddMsgOfInfoV2LongTitle("1.任务池 - <a href='TaskPoolApply.aspx'>》2.申请下来工作</a>",
                    "当前任务池里没有待办工作......");
                return;
            }
            ...
            if (this.GroupBy != "FlowName")
                this.Pub1.AddTDTitle("<a href='TaskPoolSharing.aspx?GroupBy=FlowName" + extStr + "&T=" + this.timeKey + "' >流程</a>");

            if (this.GroupBy != "NodeName")
                this.Pub1.AddTDTitle("<a href='TaskPoolSharing.aspx?GroupBy=NodeName" + extStr + "&T=" + this.timeKey + "' >节点</a>");

            if (this.GroupBy != "StarterName")
                this.Pub1.AddTDTitle("<a href='TaskPoolSharing.aspx?GroupBy=StarterName" + extStr + "&T=" + this.timeKey + "' >发起人</a>");

                this.Pub1.AddTDTitle("<a href='TaskPoolSharing.aspx?GroupBy=PRI" + extStr + "&T=" + this.timeKey + "' >优先级</a>");
            ...
            foreach (string g in gVals)
            {
                ...
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    {
                        if (isRead == 0)
                            this.Pub1.AddTD("Class=TTD onclick=\"SetImg('" + appPath + "','I" + gIdx + "_" + i + "')\"", "<a href=\"javascript:WinOpenIt('" + appPath + "WF/MyFlow.aspx?FK_Flow=" + dr["FK_Flow"] + "&FK_Node=" + dr["FK_Node"] + "&FID=" + dr["FID"] + "&WorkID=" + dr["WorkID"] + "&IsRead=0&T=" + this.timeKey + "','" + dr["WorkID"] + "');\" ><img class=Icon src='/WF/Img/Mail_UnRead.png' id='I" + gIdx + "_" + i + "' />" + dr["Title"].ToString() + "</a>");
                        else
                            this.Pub1.AddTD("Class=TTD", "<a href=\"javascript:WinOpenIt('/WF/MyFlow.aspx?FK_Flow=" + dr["FK_Flow"] + "&FK_Node=" + dr["FK_Node"] + "&FID=" + dr["FID"] + "&WorkID=" + dr["WorkID"] + "','" + dr["WorkID"] + "');\"  ><img src='/WF/Img/Mail_Read.png' id='I" + gIdx + "_" + i + "' class=Icon />" + dr["Title"].ToString() + "</a>");
                    }
                    else
                    {
                        if (isRead == 0)
                            this.Pub1.AddTD("Class=TTD onclick=\"SetImg('" + appPath + "','I" + gIdx + "_" + i + "')\" ", "<a href=\"MyFlow.aspx?FK_Flow=" + dr["FK_Flow"] + "&FK_Node=" + dr["FK_Node"] + "&FID=" + dr["FID"] + "&WorkID=" + dr["WorkID"] + "&IsRead=0&T=" + this.timeKey + "\" ><img class=Icon src='/WF/Img/Mail_UnRead.png' id='I" + gIdx + "_" + i + "' />" + dr["Title"].ToString() + "</a>");
                        else
                            this.Pub1.AddTD("Class=TTD ", "<a href=\"MyFlow.aspx?FK_Flow=" + dr["FK_Flow"] + "&FK_Node=" + dr["FK_Node"] + "&FID=" + dr["FID"] + "&WorkID=" + dr["WorkID"] + "&T=" + this.timeKey + "\" ><img class=Icon src='/WF/Img/Mail_Read.png' id='I" + gIdx + "_" + i + "' />" + dr["Title"].ToString() + "</a>");
                    }
