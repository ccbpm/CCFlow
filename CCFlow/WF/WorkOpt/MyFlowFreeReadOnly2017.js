...
//初始化下拉列表框的OPERATION
function InitDDLOperation1(workNodeData, mapAttr, defVal) {
    var operations = '';
    //外键类型
    if (mapAttr.LGType == 2) {
...
function GenerWorkNode() {
    ...
    $.ajax({
        ...
        async: true,
        data: pageData,
        url: "Handler.ashx" + "?DoType=ViewWorkNodeFrm&m=" + Math.random() + "&" + urlParam,
        dataType: 'html',
        success: function (data) {
            ...
            for (var j = 0; j < workNodeData.Sys_MapAttr.length; j++) {
                ...
                //如果是整行的需要添加  style='clear:both'
                var defValue = ConvertDefVal(workNodeData, mapAttr.DefVal, mapAttr.KeyOfEn);
                if ($('[name=TB_' + mapAttr.KeyOfEn+']').length == 1) {
                    $('[name=TB_' + mapAttr.KeyOfEn+']').val(defValue);
                }
            }
            //设置表单只读
