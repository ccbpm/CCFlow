...
#warning zhoupeng 打开如下注释代码？没有考虑到为什么要改变PK.
		...
		protected void CreatePhysicsTable()
		{
			...
			{
				DBAccessOfOracle1.RunSQL(SqlBuilder.GenerCreateTableSQLOfOra_OK(this));
				this.CreateIndexAndPK();
				return;
			}
			if (this._enMap.EnDBUrl.DBUrlType == DBUrlType.DBAccessOfOracle2)
			{
				DBAccessOfOracle2.RunSQL(SqlBuilder.GenerCreateTableSQLOfOra_OK(this));
				this.CreateIndexAndPK();
				return;
			}
							...
							if (len < attr.MaxLength)
							{
								try
								{
									if (attr.MaxLength >= 4000)
										DBAccess.RunSQL("ALTER TABLE " + this.EnMap.PhysicsTable + " ALTER column " + attr.Key + "  nvarchar(MAX) ");
									else
