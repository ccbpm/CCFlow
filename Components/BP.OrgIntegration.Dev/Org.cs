...
namespace BP.OrgIntegration
{
    ...
    public class OrgAttr : BP.En.EntityNoNameAttr
    {
        ...
        /// 异步导出任务id
        /// </summary>
        public const string AsyncTaskID = "AsyncTaskID";
    ...
    public class Org : EntityNoName
    {
        ...
        public string AsyncTaskID
        {
            get
            {
                return this.GetValStringByKey(OrgAttr.AsyncTaskID);
            }
            set
            {
                this.SetValByKey(OrgAttr.AsyncTaskID, value);
            }
        }
        ...
        public override Map EnMap
        {
            get
            {
                ...
                map.AddTBInt(OrgAttr.OrgSta, 1, "是否启用组织", true, false);
                #endregion 字段
                ...
                map.AddTBString(OrgAttr.AsyncTaskID, null, "异步导出通讯录id", false, false, 0, 4000, 36);
        ...
        protected override bool beforeUpdateInsertAction()
        {
          ...
          //  this.PinYin = "," + pinyinQP + "," + pinyinJX + ",";
            BP.Sys.Base.Glo.WriteUserLog("新建/修改组织:" + this.ToJson(), "组织数据操作");
        ...
        /// 初始化数据.
        /// </summary>
        public void Init_OrgDatas(bool isQYWX = false)
        {
            #region 初始化 - 流程树.
            ...
            #region 表单树.
            BP.WF.Template.FlowFormTree ft = new BP.WF.Template.FlowFormTree();
            ft.No = this.No; //公司编号
            ft.Name = "表单树";
            ft.OrgNo = this.No;
            ...
            #region 初始化部门.
            //根目录.
            BP.Cloud.Dept dept = new Cloud.Dept(); 
            dept.No = this.No;
            dept.ParentNo = "100";
            dept.Name = this.Name;
            dept.OrgNo = this.No;
            ...
            BP.Port.Emp ep = new BP.Port.Emp(this.No + "_" + this.Adminer);
            ep.DeptNo = this.No;
            ...
            //设置主键.
            myde.MyPK = dept.No + "_" + this.Adminer;
            ...
            if (isQYWX == false)
            {
                ...
                BP.Port.StationType st = new StationType();
            ...
            //return "url@/Admin/Portal/Default.htm?SID=" + ep.SID + "&UserNo=" + ep.No;
        }
        /// <summary>
        /// 生成一个唯一的GUID》
        /// </summary>
        ...
        public static string GenerNewOrgNo()
        {
            while (true)
            {
                string orgNo = System.Guid.NewGuid().ToString().Substring(0, 4).Replace("_", "");

                string sql = "SELECT count(No) as Num FROM Port_Org WHERE No='" + orgNo + "'";
                if (DBAccess.RunSQLReturnValInt(sql) >= 1)
                    continue;

                sql = "SELECT count(No) as Num FROM Port_Dept WHERE No='" + orgNo + "'";
                if (DBAccess.RunSQLReturnValInt(sql) >= 1)
                    continue;
        ...
        {
            BP.Sys.Base.Glo.WriteUserLog("删除组织Org:" + this.ToJson(), "组织数据操作");
