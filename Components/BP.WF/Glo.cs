...
namespace BP.WF
{
    ...
    public class Glo
    {
        ...
        /// 2, 审核意见.
        /// 3, 附件.
        /// 4, 做成zip文件，放在 \DataUser\Temp 下面可供下载.
        /// </summary>
        public static void MakeHtmlDocumentOfFoolFrm(string frmID, Int64 workid,string flowNo=null)
        {
            ...
            //把模版文件copy过去.
            string templateFile = SystemConfig.PathOfDataUser + "\\InstancePacketOfData\\Template\\index.htm";
            System.IO.File.Copy(templateFile, path + "\\index.htm",true);

         //   System.IO.File.Copy(, path + "\\index.htm");

            string docs = BP.DA.DataType.ReadTextFile(templateFile);
            ...
            #region 把字段信息写入到sb 里面.
            StringBuilder frmInfo = new System.Text.StringBuilder();
            frmInfo.Append("<table width='100%'>");
            ...
            foreach (MapAttr attr in attrs)
            {
                ...
                if (attr.UIIsLine == true)
                {
                    if (isLeft == false)
                        frmInfo.Append("</tr>");

                    frmInfo.Append("<tr>");
                    frmInfo.Append("<td>" + attr.Name + "</td>");
                    frmInfo.Append("<td colspan=3>" + wk.GetValStrByKey(attr.KeyOfEn) + "</td>");
                    frmInfo.Append("</tr>");
                ...
                if (attr.LGType == FieldTypeS.Enum || attr.LGType == FieldTypeS.FK)
                    frmInfo.Append("<td>" + attr.GetValRefTextByKey(attr.KeyOfEn) + "</td>");

                if (attr.LGType == FieldTypeS.Normal)
                    frmInfo.Append("<td>" + attr.GetValByKey(attr.KeyOfEn) + "</td>");
            ...
            #region 生成附件信息.
            string sql = "SELECT Sort, FileName, FileExts, FileSize from Sys_FrmAttachmentDB WHERE RefPKVal=" + workid;
            DataTable dt = DBAccess.RunSQLReturnTable(sql);
            if (dt.Rows.Count == 0)
            {
                string info = "<ul>";
                foreach (DataRow dr in dt.Rows)
                {
                    string filePath = dr["FileFullName"].ToString();

                    info += "<li><a href='" + dr["FileName"] + "' >" + dr["FileName"] + "</a></li>";
                }
                info += "</ul>";

                frmInfo.Append("<tr>");
                frmInfo.Append("<td colspan=4>" + info + "</td>");
                frmInfo.Append("</tr>");
            }
            #endregion
            ...
            if (flowNo != null)
            {
                sql = "SELECT * FROM ND" + int.Parse(flowNo) + "Truck WHERE WorkID=" + workid + " AND ActionType=" + (int)ActionType.WorkCheck + " ORDER RDT ";
                dt = DBAccess.RunSQLReturnTable(sql);
                foreach (DataRow dr in dt.Rows)
                {
                    frmInfo.Append("<tr>");
                    frmInfo.Append("<td>" + dr["NDFromT"] + "</td>");

                    string msg = dr["Msg"].ToString();
                    msg += "<br>";
                    msg += "审核人:" + dr["NDFromT"] + " &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;审核日期:" + dr["RDT"].ToString();
                    frmInfo.Append("<td colspan=3>" + msg + "</td>");
                    frmInfo.Append("</tr>");
                }
            }
            #endregion 生成审核信息.

            frmInfo.Append("</table>");
            #endregion 把字段信息写入到数据库.

            //模版文件.
            docs = BP.DA.DataType.ReadTextFile(SystemConfig.PathOfDataUser + "\\InstancePacketOfData\\Template\\index.htm");
            docs = docs.Replace("@FrmInfo", frmInfo.ToString());

            string file = SystemConfig.PathOfDataUser + "\\InstancePacketOfData\\" + frmID + "\\" + workid + ".htm";
            BP.DA.DataType.WriteFile(file, docs);
        }
        /// <summary>
        /// 生成一个可以归档的文件包
        /// </summary>
