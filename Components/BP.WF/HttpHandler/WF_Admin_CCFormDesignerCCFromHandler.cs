            ...
            try
            {
                ...
                switch (this.DoType)
                {
                    ...
                    case "SaveForm": //保存表单数据.
                        try
                        {
                            string diagram = this.GetRequestVal("diagram");//表单 H5 格式.
                    ...
                    case "LetLogin":    //使管理员登录
                        if (WebUser.No == null)
                        {
                            msg = "err@登录信息丢失，请重新登录。";
                        }

                        if (WebUser.No != "admin")
                        {
                            msg = "err@当前的登录人员已经改变，请重新使用admin登录。";
                    ...
                    case "GetSFTableList": //获取所有的外键表.
                        string pageNumberStr = this.GetRequestVal("pageNumber");
                        int pageNumber = 1;
                    ...
                    case "HiddenFieldDelete": //删除隐藏字段.
                        string records = this.GetRequestVal("records");
                        ...
                        msg = result.ToString();
                        break;
                    case "CcformElements"://杨玉慧 获取表单元素的 JSON 字符串
