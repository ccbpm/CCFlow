...
using BP.Tools;
using BP.En;
using BP.Port.DINGTalk.DDSDK;
using BP.DA;
using BP.Port.DINGTalk.Utility;

namespace BP.Port.DINGTalk
{
    /// <summary>
    /// 钉钉主类
    /// </summary>
                        ...
                        if (userList != null)
                        {
                            foreach (DepartMentUserInfo userInfo in userList.userlist)
                            {
                                ...
                                if (emp.IsExit(EmpAttr.No, userInfo.userid) == true)
                                {
                                    ...
                                    deptEmp.DeptNo = deptMentInfo.id;
...
//nameOfPath.Do();
                ...
                foreach (Dept gpm_Dept in gpm_Depts)
                {
                    ...
                    else
                    {
                        ...
                        foreach (DeptEmp deptEmp in deptEmps)
                        {
                            ...
                            foreach (DepartMentUserInfo userInfo in userList.userlist)
                            {
                                if (deptEmp.EmpNo.Equals(userInfo.userid))
                                {
                                    isHave = true;
                                    break;
                                }
        ...
        public CreateUser_PostVal GPM_Ding_CreateEmp(Emp emp)
        {
            ...
            try
            {
                IDictionary<string, object> list = new Dictionary<string, object>();
                //如果用户编号存在则按照此账号进行新建
                if (!(DataType.IsNullOrEmpty(emp.No) || DataType.IsNullOrEmpty(emp.No)))
                {
