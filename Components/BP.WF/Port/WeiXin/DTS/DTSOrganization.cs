...
namespace BP.Port.WeiXin.DTS
{
    public class DTSOrganization : Method
    {
        ...
        public override object Do()
        {
            #region 读取数据.
            //判断是否配置了企业号
            if (BP.DA.DataType.IsNullOrEmpty(BP.Difference.SystemConfig.WX_CorpID))
            ...
            foreach (DeptEntity deptMent in DeptMentList.department)
            {
                ...
                foreach (UserEntity userInfo in users.userlist)
                {
                    ...
                    if (emps.Retrieve(EmpAttr.No, userInfo.userid) > 0)
                    {
                        ...
                        deptEmp.EmpNo = userInfo.userid;
                        deptEmp.DeptNo = deptMent.id;
                        deptEmp.Insert();
                    }
                    //如果没有，默认主部门是当前第一个
                    else
