            ...
            get
            {
               ...
               // str += "@0=01.按岗位智能计算";
                ...
                map.AddDDLSysEnum(NodeAttr.FormType, (int)NodeFormType.FreeForm, "节点表单方案", true, true,
                 "NodeFormType", "@0=傻瓜表单(ccflow6取消支持)@1=自由表单@2=嵌入式表单@3=SDK表单@4=SL表单(ccflow6取消支持)@5=表单树@6=动态表单树@7=公文表单(WebOffice)@8=Excel表单(测试中)@9=Word表单(测试中)@100=禁用(对多表单流程有效)");
       ...
       //true, true, NodeAttr.WhenNoWorker, "@0=提示错误@1=自动转到下一步");
          ...
          true, true, BtnAttr.SelectAccepterEnable);
               ...
               // map.AddTBFloat(NodeAttr.Workload, 0, "工作量(单位:小时)", true, false);
                ...
                #region 父子流程
                map.AddDDLSysEnum(FrmSubFlowAttr.SFSta, (int)FrmSubFlowSta.Disable, "控件状态",
        ...
        {
            return Glo.CCFlowAppPath + "WF/Admin/FoolFormDesigner/Designer.aspx?PK=ND" + this.NodeID;
        }
        /// <summary>
        /// 设计自由表单
        /// </summary>
        ...
        {
            return Glo.CCFlowAppPath + "WF/Admin/FoolFormDesigner/CCForm/Frm.aspx?FK_MapData=ND" + this.NodeID + "&FK_Flow=" + this.FK_Flow;
        }
        /// <summary>
        /// 绑定独立表单
        /// </summary>
        ...
        public string DoMapData()
        {
            ...
            switch (type)
            {
                case NodeFormType.FreeForm:
                    PubClass.WinOpen(Glo.CCFlowAppPath + "WF/Admin/FoolFormDesigner/CCForm/Frm.aspx?FK_MapData=ND" + this.NodeID + "&FK_Flow=" + this.FK_Flow, "设计表单", "sheet", 1024, 768, 0, 0);
                    break;
                default:
                case NodeFormType.FixForm:
                    PubClass.WinOpen(Glo.CCFlowAppPath + "WF/Admin/FoolFormDesigner/Designer.aspx?PK=ND" + this.NodeID, "设计表单", "sheet", 800, 500, 210, 300);
                    break;
            }
            return null;
        }
