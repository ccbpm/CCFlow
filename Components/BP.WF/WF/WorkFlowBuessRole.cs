...
namespace BP.WF
{
    ...
    public class WorkFlowBuessRole
    {
        ...
        public static string GenerTitle(Flow fl, Work wk, Emp emp, string rdt)
        {
            ...
            titleRole = titleRole.Replace("@WebUser.FK_DeptName", emp.DeptText);
            titleRole = titleRole.Replace("@WebUser.FK_Dept", emp.DeptNo);
        ...
        public static string GenerTitle(Flow fl, Work wk)
        {
            ...
            titleRole = titleRole.Replace("@FlowName", fl.Name);
...
#warning 为什么，加这个代码？牺牲了很多效率，我注销了. by zhoupeng 2016.8.15
        ...
        public static string GenerBillNo(string billNo, Int64 workid, Entity en, string flowPTable)
        {
           ...
           // sql += " ORDER BY BillNo ";
            ...
            int ilsh = 0;
            if (DataType.IsNullOrEmpty(maxBillNo)==true)
            {
                //没有数据，则所有流水号都从1开始
        ...
        public static DataTable RequetNextNodeWorkers(Flow fl, Node currNode, Node toNode, Entity enParas, Int64 workid)
        {
            if (toNode.ItIsGuestNode)
            {
                ...
                return mydt;
                /////xxx
                //DBAccess.GetFileFromDB("c:\\xxx.xml", "Sys_MapData", "No", "Frm_VstoFxx", "VSTOFile");
                ////xxxxx
                //DBAccess.SaveFileToDB("c:\\xxx.xml", "Sys_MapData", "No", "Frm_VstoFxx", "VSTOFile");
            }
                ...
                || toNode.HisDeliveryWay == DeliveryWay.ByStarter)
            {
                ...
                foreach (string nd in ndStrs)
                {
                    ...
                    if (currNode.ItIsSubThread == true)
                        ps.Add("OID", workid);
                    else
                        ps.Add("OID", workid);
                ...
                || toNode.HisDeliveryWay == DeliveryWay.ByPreviousNodeFormStationsOnly)
            {
                ...
                {
                    string deptNo = BP.Web.WebUser.DeptNo;
            ...
            if (toNode.HisDeliveryWay == DeliveryWay.ByTeamDeptOnly)
            {
                ...
                ps = new Paras();
                ps.Add("FK_Node", toNode.NodeID);
                ps.Add("FK_Dept", BP.Web.WebUser.DeptNo);
           ...
           /* #region 按角色计算(以部门集合为纬度).
            ...
            if (currNode.ItIsStartNode == false)
            {
                ps = new Paras();
                dt = DBAccess.RunSQLReturnTable(ps);
                // 如果能够找到.
