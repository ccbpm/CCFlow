    ...
    public class WorkNode
    {
        ...
        private GenerWorkerLists InitWorkerLists(WorkNode town, DataTable dt, Int64 fid = 0)
        {
            ...
            if (dt.Rows.Count == 1)
            {
                ...
                if (emp.RetrieveFromDBSources() == 0)
                {
                    string[] para = new string[1];
                    para[0] = wl.FK_Emp;
                    string str = BP.WF.Glo.multilingual("@设置接收人规则错误, 接收人[{0}]不存在或者被停用。", "WorkNode", "invalid_setting_receiver", para);
        ...
        private void CC(Node node)
        {
            ...
            if (this.HisNode.HisCCRole == CCRole.HandCC)
            {
                ...
                if (cclist.Count > 0)
                {
                    ps.SQL = "UPDATE WF_CCList SET Sta=0, RDT=" + SystemConfig.AppCenterDBVarStr + "RDT,Title=" + SystemConfig.AppCenterDBVarStr + "Title  WHERE  WorkID=" + SystemConfig.AppCenterDBVarStr + "WorkID AND FK_Node=" + SystemConfig.AppCenterDBVarStr + "FK_Node ";
        ...
        public Nodes Func_GenerNextStepNodes()
        {
            ...
            #region 如果使用户选择的.
            if (this.HisNode.CondModel == DirCondModel.ByDDLSelected
        ...
        private void NodeSend_11(Node toND)
        {
            ...
            if (this.HisWork.EnMap.PhysicsTable != toWK.EnMap.PhysicsTable)
            {
                ...
                {
                    Log.DefaultLogWriteLineInfo(BP.WF.Glo.multilingual("@出现SQL异常! 可能是没有修复表或者重复发送.详情:{0}.", "WorkNode", "sql_exception_1", ex.Message));
                    try
                    {
                    ...
                    {
                        Log.DefaultLogWriteLineInfo(BP.WF.Glo.multilingual("@保存工作出错! 详情:{0}.", "WorkNode", "error_saving_data", ex1.Message));
            ...
            {
                //Img 的路径问题.
                this.addMsg(SendReturnMsgFlag.AllotTask, null, "<a href='./WorkOpt/AllotTask.htm?WorkID=" + this.WorkID + "&FK_Node=" + toND.NodeID + "&FK_Flow=" + toND.FK_Flow + "'  target=_self><img src='" + SystemConfig.HostURLOfBS + "/WF/Img/AllotTask.gif' border=0/>指定特定的处理人处理</a>。", SendReturnMsgType.Info);
            }
            ...
            {
                if (this.HisNode.IsStartNode)
                    this.addMsg(SendReturnMsgFlag.ToEmpExt, null, "@<a href='./WorkOpt/UnSend.htm?DoType=UnSend&UserNo=" + WebUser.No + "&SID=" + WebUser.SID + "&WorkID=" + this.HisWork.OID + "&FK_Flow=" + toND.FK_Flow + "' ><img src='" + SystemConfig.HostURLOfBS + "/WF/Img/Action/UnSend.png' border=0/>撤销本次发送</a>", SendReturnMsgType.Info);
                else
                    this.addMsg(SendReturnMsgFlag.ToEmpExt, null, "@<a href='./WorkOpt/UnSend.htm?DoType=UnSend&UserNo=" + WebUser.No + "&SID=" + WebUser.SID + "&WorkID=" + this.HisWork.OID + "&FK_Flow=" + toND.FK_Flow + "' ><img src='" + SystemConfig.HostURLOfBS + "/WF/Img/Action/UnSend.png' border=0 />撤销本次发送</a> ", SendReturnMsgType.Info);
            }
...
#warning 记录日志.
        ...
        public void CheckFrm1ToN()
        {
            ...
            foreach (FrmNode fn in frmNDs)
            {
                ...
                foreach (GenerWorkerList item in current_gwls)
                {
                    ...
                    //    {
                    //        geEn.SetValByKey(attr.Key, DataType.CurrentDataTime);
                    //        continue;
                    //    }
        ...
        public void CheckFrmHuiZongToDtl()
        {
            ...
            foreach (FrmNode fn in frmNDs)
            {
                ...
                dtlEn.OID = (int)this.WorkID;
                dtlEn.RDT = DataType.CurrentDataTime;
                ...
                foreach (FrmNode item in nds)
                {
                    ...
                    foreach (MapAttr mapAttr in mapAttrs)
                    {
                        ...
                        if (str == null || DataType.IsNullOrEmpty(str) == true || str.Trim() == "")
                            err += BP.WF.Glo.multilingual("@表单{0}字段{1},{2}不能为空.", "WorkNode", "form_field_must_not_be_null_1", item.FK_Frm, mapAttr.KeyOfEn, mapAttr.Name);
                    }
                }
        ...
        public bool DealTeamUpNode()
        {
            ...
            foreach (GenerWorkerList gwl in gwls)
            {
                ...
                //写入日志.
                if (this.HisGenerWorkFlow.HuiQianTaskSta != HuiQianTaskSta.None)
                    this.AddToTrack(ActionType.TeampUp, gwl.FK_Emp, todoEmps, this.HisNode.NodeID, this.HisNode.Name, BP.WF.Glo.multilingual("会签", "WorkNode", "cross_signing"));
                else
                    this.AddToTrack(ActionType.TeampUp, gwl.FK_Emp, todoEmps, this.HisNode.NodeID, this.HisNode.Name, BP.WF.Glo.multilingual("协作发送", "WorkNode", "cross_signing"));
        ...
        private SendReturnObjs NodeSendToYGFlow(Node node, string toEmpIDs)
        {
            ...
            if (DataType.IsNullOrEmpty(toEmpIDs))
            {
                ...
                {
                    sql = "SELECT A.FK_Emp No FROM Port_TeamEmp A, WF_NodeTeam B, Port_Emp C WHERE A.FK_Emp=C." + BP.Sys.Glo.UserNoWhitOutAS + " AND A.FK_Team=B.FK_Team AND B.FK_Node=" + dbStr + "FK_Node AND C.FK_Dept=" + dbStr + "FK_Dept ORDER BY A.FK_Emp";
                    ps = new Paras();
                    ps.Add("FK_Node", node.NodeID);
                ...
                {
                    sql = "SELECT A.FK_Emp No FROM Port_TeamEmp A, WF_NodeTeam B, Port_Emp C WHERE A.FK_Emp=C." + BP.Sys.Glo.UserNoWhitOutAS + " AND A.FK_Team=B.FK_Team AND B.FK_Node=" + dbStr + "FK_Node AND C.OrgNo=" + dbStr + "OrgNo ORDER BY A.FK_Emp";
                    ps = new Paras();
                    ps.Add("FK_Node", node.NodeID);
        ...
        public SendReturnObjs NodeSend(Node jumpToNode, string jumpToEmp, bool IsReturnNode = false)
        {
                ...
                this.HisGenerWorkFlow.TodoEmps.Contains(WebUser.No + ",") == false)
            {
                ...
                //throw new Exception(BP.WF.Glo.multilingual("@当前工作您已经处理完成，或者您({0} {1})没有处理当前工作的权限.", "WorkNode", "current_work_completed", this.Execer, this.ExecerName));
            }
            #endregion 安全性检查.
            ...
            if (this.IsStopFlow == true)
            {
                ...
                /*if (this.HisGenerWorkFlow.PWorkID != 0 && this.HisFlow.IsToParentNextNode == true)
                {
                    ...
                    if (DBAccess.RunSQLReturnValInt(mysql, 0) != 1)
                    {
            ...
            try
            {
                ...
                if (this.HisGenerWorkFlow.WFState == WFState.ReturnSta)
                {
                    ...
                    int returnNodeID = int.Parse(mydt.Rows[0][0].ToString());
                    int isBackTracking = int.Parse(mydt.Rows[0]["IsBackTracking"].ToString());
...
#warning 如果这里设置了自动跳转，现在去掉了. 2014-11-07.
        ...
        private void InitStartWorkDataV2()
        {
            ...
            if (this.title == null)
            {
                ...
                {
                    /*如果是外部参数,*/
                    this.HisGenerWorkFlow.Title = DBAccess.RunSQLReturnString("SELECT Title FROM " + this.HisFlow.PTable + " WHERE OID=" + this.WorkID);
                    if (DataType.IsNullOrEmpty(this.HisGenerWorkFlow.Title))
                        this.HisGenerWorkFlow.Title = BP.WF.Glo.multilingual("{0},{1}在{2}发起", "WorkNode", "start_wf_title", this.Execer, this.ExecerName, DataType.CurrentDataTimeCN);
            ...
            this.HisGenerWorkFlow.RDT = DataType.CurrentDataTimess;  // this.HisWork.RDT;
            ...
            if (this.HisFlow.SDTOfFlowRole == SDTOfFlowRole.ByAllNodes)
            {
                ...
                this.HisGenerWorkFlow.SDTOfFlow = sdtOfFlow.ToString(DataType.SysDataTimeFormat);
            ...
            if (this.HisFlow.SDTOfFlowRole == SDTOfFlowRole.ByDays)
            {
                ...
                this.HisGenerWorkFlow.SDTOfFlow = DateTime.Now.AddDays(day).ToString(DataType.SysDataTimeFormat);
            }
            //加入两个参数. 2013-02-17
