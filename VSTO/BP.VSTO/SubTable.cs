...
namespace BP.VSTO
{
    ...
    public class SubTable
    {
        ...
        public SubTable(Excel.Range range, DataTable data, Dictionary<int, string> columns, int intTableHeadHeight, string strPkColumnName)
        {
            ...
            //else
            //{
            //    var beginRowIndex = range.Row + this.TableHeadHeight;
            //    var correctFirstRowIndex = _newData.Rows.Count == 0 ? true : Equals(_newData.Rows[0]["Idx"], beginRowIndex) ? true : false;

            //    foreach (DataRow dr in _newData.Rows) //!若Excel表单数据与数据库数据不一致，则需修改此方法
            //    {
            //        if (correctFirstRowIndex)
            //            this.SetConnection(Convert.ToInt32(dr["Idx"]), _newData.Rows.IndexOf(dr));
            //        else
            //            this.SetConnection(beginRowIndex++, _newData.Rows.IndexOf(dr));
            //    }
            //}
        }
        ...
        public void RefreshConnection()
        {
            ...
            {
                if (Equals(dr.GetColumnError("Idx"), RowStatus.Deleted.ToString()))
                    continue;
        ...
        public void RefreshData(DataTable newData, Excel.Range range)
        {
            //排序
            if (newData.Columns.Contains("Idx"))
            {
                newData.DefaultView.Sort = "Idx ASC";
            ...
            this._Ro = range.Row;
            this._rangeRows = range.Rows.Count;
            this._originData = newData.Copy();
            this._newData = newData.Copy();
            this._name = newData.TableName;
        ...
        /// <returns></returns>
        public string GetOidByRowid(int rowIdx)
        {
            return _range.Worksheet.Cells[rowIdx, _range.Column].ID as string;
        }
        ...
        /// <summary>
        /// 获取当前行之前的与当前行具有相同主键值的行号（用于查询由复制行操作而产生的新生）
        /// </summary>
        /// <param name="pkValue">主键值</param>
        /// <param name="currentRowIndex">当前行号</param>
        /// <returns>返回0表示未找到</returns>
        public int GetSameRowBeforeCurrentRowByPkValue(string pkValue, int currentRowIndex)
        {
            var beginRowIndex = _range.Row + _TableHeadHeight;

            for (var r = beginRowIndex; r < currentRowIndex; r++)
            {
                if (Equals(_range.Worksheet.Cells[r, _range.Column].ID, pkValue))
                    return r;
            }
        ...
        public void DeleteRow(int Idx, Excel.Range range = null)
        {
            ...
            _newData.Select(string.Format("OID='{0}'", _rowsConnection[Idx]))[0].SetColumnError("Idx", RowStatus.Deleted.ToString());
            //更改行关联
        ...
        private void UndoDeleteRow(int Idx, string rowPk, Excel.Range range = null)
        {
            ...
            _newData.Select(string.Format("OID='{0}'", _rowsConnection[Idx]))[0].SetColumnError("Idx", RowStatus.Origin.ToString());
