...
namespace CCFormExcel2013
{
	...
	{
		#region 全局变量
		private CCFormExcel2013.CCFormAPI.CCFormAPISoapClient client;
		...
		public Dictionary<string, string> InitTesterArgsString()
		{
			...
			string argstr = "excelform://-fromccflow,App=FrmExcel,DoType=Frm_Init,FK_MapData=FX_JNHBG_64_1B,IsEdit=1,IsPrint=0,WorkID=4351,FK_Flow=003,FK_Node=301,UserNo=duqinglian,FID=0,SID=gwkqt2oadrkhltoqlvsuueuj,PWorkID=0,IsLoadData=1,PFlowNo=,Frms=FX_JNHBG_64_1B,IsCheckGuide=1,e1m=0.9048208584001134,WSUrl=http://localhost:28048/WF/CCForm/CCFormAPI.asmx";
		...
		private void ThisAddIn_Startup(object sender, System.EventArgs e)
		{
			...
			else if (Application.Version == "14.0")
			{
				MessageBox.Show("检测到您正在使用Excel 2010，但您当前安装的CCFlow插件为Excel 2013专版，继续使用可能会出现问题，推荐您安装Excel 2010专版插件。");
			}
		...
		public bool SetDtlData(DataTable dt, bool isFill = true) //x尚未测试
		{
			...
			else
			{
				...
				htColumns = GetAreaColumns(range, out int TableHeadHeight);
