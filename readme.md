...
1. D:\ccflow.git\Components  组件目录.
2. D:\ccflow.git\Components\BP.En30   -- 底层基类.
3. D:\ccflow.git\Components\BP.Web.Controls30   --bs控件层.
4. D:\ccflow.git\Components\BP.WF  --工作流程引擎层
5. D:\ccflow.git\DLL  -- 第三方组件中需要调用dll.
6. D:\ccflow.git\Docs - 文档
7. D:\ccflow.git\CCFlow  -- BS工作流程引擎前台.  
8. D:\ccflow.git\CCFlowServices - 流程服务程序. 
9. D:\ccflow.git\DemoAndTesting  - 单元测试&Demo
10. D:\ccflow.git\DesignerForSilverlight - 流程设计器，表单设计器.


## ccflow前台目录结构.
   
  ~前台程序.(不建议用户修改，如果修改请提交给我们，否则您就没有办法升级.)~

1. D:\ccflow.git\CCFlow\WF\ --前台程序.
2. D:\ccflow.git\CCFlow\WF\Comm  --通用功能层.
3. D:\ccflow.git\CCFlow\WF\Data  -- 应用程序数据目录. 包含一些xml,等等。
4. D:\ccflow.git\CCFlow\WF\Data\Install 与安装有关系的文件
5. D:\ccflow.git\CCFlow\WF\Data\JSLib  系统javascript 函数库。
6. D:\ccflow.git\CCFlow\WF\Data\Language 语言包(完善中)
7. D:\ccflow.git\CCFlow\WF\Data\Node  cs流程设计器节点类型（cs流程设计器不在发展）
8. D:\ccflow.git\CCFlow\WF\Data\XML  xml配置文件不仅仅ccflow使用,bp框架也使用它。
9. D:\ccflow.git\CCFlow\WF\UC  --用户控件.
10. D:\ccflow.git\CCFlow\WF\DocFlow -- 公文流程(目前还不是很完善)
11. D:\ccflow.git\CCFlow\OA -- 通用oa 组件(目前还不是很完善)
12. D:\ccflow.git\CCFlow\SDKFlowDemo -- sdk开发模式的案例.
13. D:\ccflow.git\CCFlow\WF\Admin - 对ccflow的管理比如设计方向条件.报表定义...
14. D:\ccflow.git\CCFlow\WF\Admin\XAP CCFlowDesigner.xap流程设计器，CCForm.xap表单设计器。   
15. D:\ccflow.git\CCFlow\WF\MapDef - 表单定义.
16. D:\ccflow.git\CCFlow\WF\SDKComponents  --流程组件目录.
17. D:\ccflow.git\CCFlow\WF\WorkOpt -- 工作处理器的附件功能.
18. D:\ccflow.git\CCFlow\WF\Admin\CCBPMDesigner -- H5的流程设计器.
19. D:\ccflow.git\CCFlow\WF\Admin\CCFormDesigner -- H5的表单设计器.
20. D:\ccflow.git\CCFlow\WF\WebOffice -- 公文流程处理程序.


  ~1.2 前台的用户数据文件，用户可以更改.~

1. D:\ccflow.git\CCFlow\DataUser --用户文件.
2. D:\ccflow.git\CCFlow\DataUser\Seal -- 电子盖章图片.
3. D:\ccflow.git\CCFlow\DataUser\UploadFile - 上传附件
4. D:\ccflow.git\CCFlow\DataUser\Style -- 个性化风格文件.
5. D:\ccflow.git\CCFlow\DataUser\CyclostyleFile -- 单据模版文件.
6. D:\ccflow.git\CCFlow\DataUser\EmailTemplete -邮件模版文件.
7. D:\ccflow.git\CCFlow\DataUser\ICON --ICON
8. D:\ccflow.git\CCFlow\DataUser\TaoHong --公文套红.
9. D:\ccflow.git\CCFlow\DataUser\Bill  单据打印生成数据.
10. D:\ccflow.git\CCFlow\DataUser\CyclostyleFile 单据模板数据
11. D:\ccflow.git\CCFlow\DataUser\DtlTemplete  导入明细表模板文件.
12. D:\ccflow.git\CCFlow\DataUser\EmailTemplete  自定义邮件发送格式文件.
13. D:\ccflow.git\CCFlow\DataUser\JSLib 用户自定义函数库
14. D:\ccflow.git\CCFlow\DataUser\JSLibData 用户自定义函数生成文件。
15. D:\ccflow.git\CCFlow\DataUser\Log 系统日志文件
16. D:\ccflow.git\CCFlow\DataUser\ReturnLog 退回日志文件.
17. D:\ccflow.git\CCFlow\DataUser\Siganture 签名文件.
18. D:\ccflow.git\CCFlow\DataUser\Style 用户自定义风格文件。
19. D:\ccflow.git\CCFlow\DataUser\UploadFile 表单附件上传文件，单附件，与多附件。
20. D:\ccflow.git\CCFlow\DataUser\XML 用户系统配置文件。

   ~1.3 权限控制系统. D:\ccflow.git\CCFlow\GPM ~
...
1. 安装设置CCFLOW前，为了保证显示效果，请确认已经安装了 D:\ccflow.git\Tools 下面的iewebcontrols.msi及owc11.exe   office组件

2. 安装微软silverlight，目前微软银光的版本为4.0，可以从  http://www.microsoft.com/silverlight/   下载并安装 ,如果你本机上是4.0以下的版本请删除它在安装它。
...
0. 确认升级到IE7 以上，并且做如下设置.
   ...
   1.1. 菜单->工具->Internet 选项-> 隐私 -> 把打开弹出窗口阻止程序 关闭上，否则一些功能就不能使用.

   1.2. 菜单->工具->Internet 选项-> 常规 -> 浏览历史记录设置-> 选中 () 每次访问此网页时. 单选按钮. 点确定，以避免缓存对程序有影响。 

   1.3. 确认你是以administrator 超级用户进行安装的。
...
2. 打开解决方案文件并编译它. CCFlow6 解决方案的位置: D:\ccflow.git\CCFlow6.sln, 设置 \WF\Admin\DBInstall.aspx为起始页.
...
4. 修改 D:\ccflow.git\CCFlow\web.config Appsetting 节点中的数据库连接。
...
5. 建立网站应用:
   ...
   5.3. 建立网站, 网站名称为CCFlow6  主目录为: D:\ccflow.git\CCFlow , 应用程序池为 ASP.NET v4.0 Classic. (建议单独建立一个应用程序池.)
...
1. 在internet 信息服务(iis) 中， web 服务扩展 asp.net v4.xxxxxx 的dll 没有被允许.
   ...
   1.1 如果找不到，就是没有安装iis的程序扩展功能。

   1.2 控制面板-> 程序和功能-> 在左边有打开或者关闭windows功能. 把 Internet information services. 与 Internet 信息服务 全部选择上.
...
4. 出现用户名及密码错误，请在web.config 文件中.
    ...
    <identity impersonate="true" userName="administrator" password="jiaozi"/> 中的 impersonate="true" 修改成 impersonate="false" 或者填写正确的密码, 也可以把  impersonate="false" 
...
8, 用户权限问题.
    ...
    如果出现:C:\Windows\Microsoft.NET\Framework64\v4.0.30319\Temporary ASP.NET Files”的写访问权限。的错误提示.  解决办法:  把“IIS_IUSERS”和你自己的系统用户赋予它操作 C：\WINDOWS\TEMP文件夹的权利，重新启动IIS，并尝试再次运行Web应用程序。
