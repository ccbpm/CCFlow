<%@ Page Title="上传文件" Language="C#" MasterPageFile="WinOpen.master" AutoEventWireup="true"
    Inherits="CCFlow.WF.CCForm.WF_CCForm_AttachmentUpload2" CodeBehind="AttachmentUpload2.aspx.cs"  ResponseEncoding="utf-8"%>
    ...
    <script language="javascript" type="text/javascript">
        ...
        function Del(fk_ath, pkVal, delPKVal) {
            if (window.confirm('您确定要删除吗？ ') == false)
                return;
            window.location.href = 'AttachmentUpload2.aspx?DoType=Del&DelPKVal=' + delPKVal + '&FK_FrmAttachment=' + fk_ath + '&PKVal=' + pkVal + '&FK_Node=<%=FK_Node %>&FK_Flow=<%=FK_Flow %>&FK_MapData=<%=FK_MapData %>&Ath=<%=Ath %>' + (GetQueryString("IsExtend")!=undefined?"&IsExtend="+GetQueryString("IsExtend"):"");
        }

        function Down(fk_ath, pkVal, delPKVal) {
            window.location.href = 'AttachmentUpload2.aspx?DoType=Down&DelPKVal=' + delPKVal + '&FK_FrmAttachment=' + fk_ath + '&PKVal=' + pkVal + '&FK_Node=<%=FK_Node %>&FK_Flow = <%=FK_Flow %>&FK_MapData=<%=FK_MapData %>&Ath=<%=Ath %>';
        }
